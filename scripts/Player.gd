...
onready var impulse_audio = $ImpulseAudio
...
func _process(delta):
	...
	highest_reached_position = (position.y if position.y < highest_reached_position 
											else highest_reached_position)
...
func _check_boundaries():
	if position.x < 0:
		position.x = 0 + half_sprite_width
	elif position.x > screen_width:
		position.x = screen_width - half_sprite_width
